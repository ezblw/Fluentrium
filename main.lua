...
local aa = {
    function()
        ...
        local function Color(hex1, hex2)
            ...
            local c1 = fromHex(hex1)
            local c2 = hex2 and fromHex(hex2) or c1
            _windowColors = { primary = c1, secondary = c2 }
        ...
        function x.CreateWindow(C, D, ...)
            ...
            elseif type(C) == 'string' then
                D = D or {}
                if type(D) == 'string' then D = { Title = D } end
                D.Title = C
                C = x
            end
            ...
            x.MinimizeKey = D.MinimizeKey or Enum.KeyCode.LeftControl
            ...
            if typeof(rawSize) == 'UDim2' then
                ...
                winSize = UDim2.fromOffset(rawSize.x or 580, rawSize.y or 460)
            end
            ...
            local function _applyGlass()
                if _glassAlpha == nil then return end
                pcall(function()
                    local alpha = math.max(0, math.min(1, _glassAlpha))
                        ...
                        elseif alpha == 0 then
                            ...
                            if colors then
                                ...
                                eg.Color = ColorSequence.new({
                                    ...
                                    ColorSequenceKeypoint.new(0.13, lc(p1,p2,0.25)),
                                    ColorSequenceKeypoint.new(0.27, lc(p1,p2,0.55)),
                                    ColorSequenceKeypoint.new(0.40, p2),
                                    ColorSequenceKeypoint.new(0.54, lc(p2,p1,0.3)),
                                    ColorSequenceKeypoint.new(0.68, lc(p1,p2,0.7)),
                                    ColorSequenceKeypoint.new(0.82, lc(p2,p1,0.15)),
            ...
            local function _fadeOut(onDone)
            ...
            local function _fadeIn()
                ...
                pcall(function()
                    ...
                    if not uiScale then
                        ...
                        uiScale.Parent = E.Root
                    end
                    uiScale.Scale = 0.88
                    _ts2:Create(uiScale, TweenInfo.new(0.2, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Scale = 1}):Play()
                end)
            ...
            if colors then
                ...
                p.GetThemeProperty = function(self, tag)
                    if tag == 'AcrylicMain' then
                        return darken(colors.primary, 0.55)
                    elseif tag == 'AcrylicGradient' then
                        return ColorSequence.new(colors.primary, colors.secondary)
                    elseif tag == 'TitleBarLine' then
                        return darken(colors.primary, 0.4)
                    elseif tag == 'Accent' then
                        return colors.secondary
                    elseif tag == 'Element' or tag == 'Tab' then
                        return darken(colors.primary, 0.6)
                    elseif tag == 'DropdownHolder' or tag == 'Dialog' or tag == 'DialogHolder' then
                        return darken(colors.primary, 0.4)
                    elseif tag == 'DropdownFrame' or tag == 'Keybind' or tag == 'Input' then
                        return darken(colors.primary, 0.65)
                    elseif tag == 'InElementBorder' or tag == 'ElementBorder' then
                        return darken(colors.secondary, 0.5)
                    end
                ...
                task.defer(function()
                    pcall(function()
                        ...
                        outerGrad.Color = ColorSequence.new({
                            ColorSequenceKeypoint.new(0,    p1),
                            ColorSequenceKeypoint.new(0.13, lerpColor(p1, p2, 0.25)),
                            ColorSequenceKeypoint.new(0.27, lerpColor(p1, p2, 0.55)),
                            ColorSequenceKeypoint.new(0.40, p2),
                            ColorSequenceKeypoint.new(0.54, lerpColor(p2, p1, 0.3)),
                            ColorSequenceKeypoint.new(0.68, lerpColor(p1, p2, 0.7)),
                            ColorSequenceKeypoint.new(0.82, lerpColor(p2, p1, 0.15)),
                            ColorSequenceKeypoint.new(1,    p1),
                        })
            ...
            local _circleGui  = nil

            local function _buildCircle()
                if _circleGui then return end
                local vp = game:GetService('Workspace').CurrentCamera.ViewportSize
                local gui = Instance.new('ScreenGui')
                gui.Name = 'FluentCircle'
                gui.ResetOnSpawn = false
                gui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
                pcall(function()
                    local fn = protectgui or (syn and syn.protect_gui) or function()end
                    fn(gui)
                end)
                gui.Parent = game:GetService('CoreGui')

                local btn = Instance.new('ImageButton')
                btn.Size = UDim2.fromOffset(circleSize, circleSize)
                btn.Position = UDim2.fromOffset(vp.X - circleSize - 24, vp.Y - circleSize - 24)
                btn.BackgroundColor3 = colors and colors.primary or Color3.fromRGB(50,50,60)
                btn.BorderSizePixel = 0
                btn.Image = circleImage
                ...
                stroke.Color = colors and colors.secondary or Color3.fromRGB(120,120,140)

                if colors then
                    local g = Instance.new('UIGradient', btn)
                    g.Color = ColorSequence.new(colors.primary, colors.secondary)
                    g.Rotation = 135
                end
            ...
            local function _doMinimize()
                ...
                if E.Minimized then
                    _buildCircle()
                    _fadeOut(function()
                        if _circleGui then _circleGui.Enabled = true end
                    end)
                else
                    if _circleGui then _circleGui.Enabled = false end
                    _fadeIn()
                end
            end
            ...
            do
                ...
                -- WindUI-style resize handle: Frame anchored at corner, half outside
                ...
                -- WindUI's exact resize icon asset, always visible at 0.5 trans
                ...
                resizeIcon.ImageTransparency = 0.5  -- always visible
                ...
                -- WindUI: brighter on hover, brightest on drag
                resizeHandle.MouseEnter:Connect(function()
                    if not _isResizing then
                        _ts3:Create(resizeIcon, TweenInfo.new(0.15), {ImageTransparency = 0.25}):Play()
                    end
                end)
                resizeHandle.MouseLeave:Connect(function()
                    if not _isResizing then
                        _ts3:Create(resizeIcon, TweenInfo.new(0.15), {ImageTransparency = 0.5}):Play()
                    end
                end)
                ...
                resizeHandle.InputBegan:Connect(function(inp)
                    ...
                    or inp.UserInputType == Enum.UserInputType.Touch then
                        ...
                        _ts3:Create(resizeIcon, TweenInfo.new(0.1), {ImageTransparency = 0.1}):Play()
                        inp.Changed:Connect(function()
                            if inp.UserInputState == Enum.UserInputState.End then
                                _isResizing = false
                                _ts3:Create(resizeIcon, TweenInfo.new(0.17), {ImageTransparency = 0.25}):Play()
                            end
                        end)
                    end
                end)
            ...
            local function _labelProxy(elem, initText)
                return setmetatable({}, {
                    __index = function(_, k)
                        if k == 'Text' then
                            return elem.TitleLabel and elem.TitleLabel.Text or initText
                        end
                        return elem[k]
                    end,
                    __newindex = function(_, k, v)
                        if k == 'Text' then
                            pcall(function() elem:SetTitle(v) end)
                        else
                            elem[k] = v
                        end
                    end,
            ...
            local function _switchWrap(fluentToggle)
                ...
                function sw:Set(bool)
                    bool = not not bool
                    pcall(function() fluentToggle:SetValue(bool) end)
                end
                return sw
            end
            ...
            local function _sliderWrap(fluentSlider, opts)
                local sd = {}
                function sd:Set(val)
                    val = tonumber(val) or opts.min or 0
                    pcall(function() fluentSlider:SetValue(val) end)
                end
                return sd
            end
            ...
            local function _dropdownWrap(fluentDD, initValues)
                ...
                function dd:Add(name)
                    table.insert(_values, tostring(name))
                    pcall(function() fluentDD:SetValues(_values) end)
                    local obj = {}
                    function obj:Remove()
                        for i, v in ipairs(_values) do
                            if v == name then table.remove(_values, i) break end
                        end
                        pcall(function() fluentDD:SetValues(_values) end)
                    end
                    return obj
                end
                function dd:Refresh(newList)
                    _values = newList or {}
                    pcall(function() fluentDD:SetValues(_values) end)
                end
                return dd
            ...
            local function _makeTabHandle(ft, myIndex)
                ...
                function tab_data:Show()
                    _win:SelectTab(myIndex)
                end

                function tab_data:AddLabel(text)
                    local elem = ft:AddParagraph({ Title = tostring(text or ''), Content = '' })
                    return _labelProxy(elem, text)
                end

                function tab_data:AddButton(text, cb)
                    ft:AddButton({ Title = tostring(text or ''), Callback = cb or function() end })
                    return {}
                end

                function tab_data:AddSwitch(text, cb)
                    local tog = ft:AddToggle(nil, {
                        Title    = tostring(text or ''),
                        Default  = false,
                        Callback = cb or function() end,
                    })
                    return _switchWrap(tog)
                end

                function tab_data:AddSlider(text, cb, opts)
                    opts = type(opts) == 'table' and opts or {}
                    local min = opts.min or 0
                    local max = opts.max or 100
                    local sl = ft:AddSlider(nil, {
                        Title    = tostring(text or ''),
                        Min      = min,
                        Max      = max,
                        Default  = opts.default or min,
                        Rounding = opts.rounding or 1,
                        Callback = cb or function() end,
                    })
                    return _sliderWrap(sl, opts)
                end

                function tab_data:AddTextBox(text, cb, opts)
                    opts = type(opts) == 'table' and opts or {}
                    ft:AddInput(nil, {
                        Title       = tostring(text or ''),
                ...
                function tab_data:AddDropdown(name, valuesOrCb, cb)
                    ...
                    if type(valuesOrCb) == 'table' then
                        ...
                        initValues = {}
                        callback   = valuesOrCb or function() end
                    end
                    local dd = ft:AddDropdown(nil, {
                        Title    = tostring(name or ''),
                        Values   = initValues,
                        Default  = nil,
                        Callback = callback,
                    })
                    return _dropdownWrap(dd, initValues)
                end
            ...
            function lib:AddTab(name)
                ...
                local myIndex = _tabCount
                if myIndex == 1 then
                    task.defer(function() _win:SelectTab(1) end)
                end
                return _makeTabHandle(ft, myIndex)
            end

            if getgenv then getgenv().library = lib end

            return lib
        end

        x.AddWindow = x.CreateWindow
                ...
                pcall(function()
                    ...
                    if nColors and nLerp and r.AcrylicPaint and r.AcrylicPaint.Frame then
                        ...
                        ng.Color = ColorSequence.new({
                            ColorSequenceKeypoint.new(0,    p1),
                            ColorSequenceKeypoint.new(0.13, nLerp(p1, p2, 0.25)),
                            ColorSequenceKeypoint.new(0.27, nLerp(p1, p2, 0.55)),
                            ColorSequenceKeypoint.new(0.40, p2),
                            ColorSequenceKeypoint.new(0.54, nLerp(p2, p1, 0.3)),
                            ColorSequenceKeypoint.new(0.68, nLerp(p1, p2, 0.7)),
                            ColorSequenceKeypoint.new(0.82, nLerp(p2, p1, 0.15)),
                            ColorSequenceKeypoint.new(1,    p1),
                        })
                        ng.Rotation = 135
    ...
    [17] = function()
        ...
        return function(t)
            ...
            function v.AddTab(O, P)
                if type(P) == 'string' then P = { Title = P } end
    ...
    [18] = function()
        ...
        function k.UpdateTheme()
            ...
            for _, hook in ipairs(k._customHooks) do
                pcall(hook)
            end
        end
    ...
    [22] = function()
        ...
        function g.New(h, i, j)
            ...
            function l.Destroy(B)
                ...
                if i ~= nil then k.Options[i] = nil end
            end
            ...
            if i ~= nil then k.Options[i] = l end
    ...
    [23] = function()
        ...
        function c.New(d, e, f)
            ...
            function h.Destroy(l)
                ...
                if e ~= nil then g.Options[e] = nil end
            end

            if e ~= nil then g.Options[e] = h end
    ...
    [26] = function()
        ...
        function c.New(d, e, f)
            ...
            -- Value textbox (WindUI style: left-aligned, 0.4 trans, TextSize 15)
            local valBox = ai('TextLabel', {
            ...
            -- Thumb: WindUI uses a Squircle pill 26×17 (ThumbSize*2 x ThumbSize+4)
            -- We approximate with UICorner. ThumbSize=13 => 26w x 17h
            local thumbHighlight = ai('Frame', {
                Size = UDim2.fromScale(1, 1),
                BackgroundTransparency = 1,
                ZIndex = 3,
            }, {
            ...
            local thumbScale = ai('UIScale', { Scale = 1 })
            ...
            -- Fill bar (Accent, 0.1 trans like WindUI)
            local fill = ai('Frame', {
            ...
            -- Rail (Text color, 0.95 trans like WindUI outer track)
            local rail = ai('Frame', {
            ...
            local function _tween(inst, t, props)
                _ts:Create(inst, TweenInfo.new(t, Enum.EasingStyle.Quint, Enum.EasingDirection.InOut), props):Play()
            end

            local function _calcAndSet(posX)
                local t = math.clamp((posX - rail.AbsolutePosition.X) / rail.AbsoluteSize.X, 0, 1)
            ...
            local function _startDrag(inp)
                ...
                -- WindUI thumb grow on drag start
                _tween(thumbScale, 0.28, { Scale = 1.3 })
                _tween(thumb, 0.28, { BackgroundTransparency = 0.85 })
                _calcAndSet(inp.Position.X)
                if _renderConn then _renderConn:Disconnect() end
                _renderConn = _rs.RenderStepped:Connect(function()
                    _calcAndSet(isTouch and inp.Position.X or af:GetMouseLocation().X)
                end)
                inp.Changed:Connect(function()
                    if inp.UserInputState == Enum.UserInputState.End then
                        i = false
                        ...
                        _tween(thumbScale, 0.2, { Scale = 1 })
                        _tween(thumb, 0.2, { BackgroundTransparency = 0 })
                    end
                end)
            end
            ...
            ah.AddSignal(thumb.InputBegan, function(p)
                if p.UserInputType == Enum.UserInputType.MouseButton1
                or p.UserInputType == Enum.UserInputType.Touch then
                    _startDrag(p)
                end
            ...
            function h.SetValue(p, s)
                ...
                _ts:Create(fill, TweenInfo.new(0.05), { Size = UDim2.fromScale(t, 1) }):Play()
                -- Thumb sits at right edge of fill bar
                thumb.Position = UDim2.new(1, 0, 0.5, 0)
                valBox.Text = tostring(p.Value)
    ...
    [27] = function()
        ...
        function c.New(d, e, f)
            ...
            -- WindUI exact dims: pill 40.8w x 24h, thumb frame 20w x 20h
            local PILL_W  = 40
            local PILL_H  = 24
            local THUMB_W = 20
            local THUMB_H = 20
            ...
            -- Pill track: Squircle approx — UICorner r=12, Text color 0.85 trans
            local pill = ai('Frame', {
                Size = UDim2.fromOffset(PILL_W, PILL_H),
                AnchorPoint = Vector2.new(1, 0.5),
                Position = UDim2.new(0, PILL_W, 0.5, 0),
            ...
            -- Layer: Accent fill, fades in when ON (WindUI: ImageColor3='Toggle' = Accent equiv)
            local layer = ai('Frame', {
                Size = UDim2.fromScale(1, 1),
                BackgroundTransparency = 1,
                Name = 'Layer',
                ThemeTag = { BackgroundColor3 = 'Accent' },
            ...
            -- Stroke: white gradient outline, fades in when ON
            local stroke = ai('Frame', {
                Size = UDim2.fromScale(1, 1),
                BackgroundTransparency = 1,
                Name = 'Stroke',
            }, {
            ...
            -- Thumb mover frame (WindUI: Frame, 20w x 20h, starts at x=2)
            local thumbFrame = ai('Frame', {
                Size = UDim2.fromOffset(THUMB_W, THUMB_H),
                Position = UDim2.new(0, OFF_X, 0.5, 0),
                AnchorPoint = Vector2.new(0, 0.5),
                BackgroundTransparency = 1,
                Name = 'Frame',
            })
            thumbFrame.Parent = pill
            ...
            bar.Parent = thumbFrame

            local function _tween(inst, t, props, es, ed)
                af:Create(inst, TweenInfo.new(t, es or Enum.EasingStyle.Quint, ed or Enum.EasingDirection.Out), props):Play()
            end

            local _isDragging = false
            local _startX, _thumbX0 = nil, nil
            local _moved = false
            ...
            function h.SetValue(m, n, _skipCb)
                n = not not n
                h.Value = n
                -- Thumb slide (WindUI: 0.15s Quint Out)
                _tween(thumbFrame, 0.15, {
                    Position = UDim2.new(0, n and ON_X or OFF_X, 0.5, 0),
                })
                -- Layer fade
                _tween(layer, 0.1, { BackgroundTransparency = n and 0 or 1 })
                -- Stroke fade
                local strokeInst = stroke:FindFirstChildOfClass('UIStroke')
                if strokeInst then
                    _tween(strokeInst, 0.1, { Transparency = n and 0.6 or 1 })
                end
                if not _skipCb then
                    g:SafeCallback(h.Callback, h.Value)
                    g:SafeCallback(h.Changed, h.Value)
            ...
            ah.AddSignal(i.Frame.InputBegan, function(inp)
                if inp.UserInputType ~= Enum.UserInputType.MouseButton1
                ...
                _thumbX0 = thumbFrame.Position.X.Offset

                -- WindUI Bar squeeze: Scale=1.5, ImageTransparency=0.85
                _tween(barScale, 0.28, { Scale = 1.5 })
                _tween(bar, 0.28, { BackgroundTransparency = 0.85 })
                ...
                _dc = _uis.InputChanged:Connect(function(mv)
                    ...
                    local nx = math.max(OFF_X, math.min(_thumbX0 + dx, ON_X))
                    -- WindUI uses 0.05s Linear for live drag
                    af:Create(thumbFrame, TweenInfo.new(0.05, Enum.EasingStyle.Linear), {
                        Position = UDim2.new(0, nx, 0.5, 0),
                    }):Play()
                    -- Live layer alpha
                    local t = (nx - OFF_X) / (ON_X - OFF_X)
                    layer.BackgroundTransparency = 1 - t
                end)
                ...
                _ec = _uis.InputEnded:Connect(function(up)
                    ...
                    _isDragging = false
                    -- WindUI bar unsqueeze: Scale=1, ImageTransparency=0
                    _tween(barScale, 0.23, { Scale = 1 })
                    _tween(bar, 0.23, { BackgroundTransparency = 0 })
                    if _dc then _dc:Disconnect(); _dc = nil end
                    if _ec then _ec:Disconnect(); _ec = nil end
                    if not _moved then
                        h:SetValue(not h.Value)
                    else
                        local mid = thumbFrame.Position.X.Offset + THUMB_W / 2
